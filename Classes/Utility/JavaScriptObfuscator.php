<?php

namespace  CodingFreaks\CfCookiemanager\Utility;


/**
 * Class JavaScriptObfuscator
 *
 * Utility class for obfuscating JavaScript code and generating obfuscated JavaScript snippets.
 *
 * @package CodingFreaks\CfCookiemanager\Utility
 */
class JavaScriptObfuscator
{
    /**
     * @var string The original JavaScript code to be obfuscated.
     */
    private $code;

    /**
     * @var string The obfuscation mask used for character substitution.
     */
    private $mask;

    /**
     * @var int The interval used for character substitution during obfuscation.
     */
    private $interval;

    /**
     * @var int The obfuscation option used for character conversion.
     */
    private $option = 0;

    /**
     * @var int The expiration time for the obfuscated code (in seconds since the Unix epoch).
     */
    private $expireTime = 0;

    /**
     * @var array An array of domain names to restrict the execution of the obfuscated code.
     */
    private $domainNames = array();

    /**
     * JavaScriptObfuscator constructor.
     * Initializes the obfuscator with a random mask, interval, and option.
     */
    function __construct()
    {
        $this->mask = $this->getMask();
        $this->interval = rand(1, 50);
        $this->option = rand(2, 8);
    }

    /**
     * Generates a random mask for character substitution.
     *
     * @return string The generated mask.
     */
    private function getMask()
    {
        $charset = str_shuffle('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ');
        return substr($charset, 0, 9);
    }


    /**
     * Applies character substitution based on the obfuscation mask.
     *
     * @param string $s The string to be obfuscated.
     * @return string The obfuscated string.
     */
    private function hashIt($s)
    {
        for ($i = 0; $i < strlen($this->mask); ++$i)
            $s = str_replace("$i", $this->mask[$i], $s);
        return $s;
    }

    /**
     * Prepares the obfuscated code by adding domain and expiration checks.
     */
    private function prepare()
    {
        if (count($this->domainNames) > 0) {
            $code = "if(window.location.hostname==='" . $this->domainNames[0] . "' ";
            for ($i = 1; $i < count($this->domainNames); $i++)
                $code .= "|| window.location.hostname==='" . $this->domainNames[$i] . "' ";
            $this->code = $code . "){" . $this->code . "}";
        }
        if ($this->expireTime > 0)
            $this->code = 'if((Math.round(+new Date()/1000)) < ' . $this->expireTime . '){' . $this->code . '}';
    }

    /**
     * Encodes the obfuscated code.
     *
     * @return string The encoded obfuscated code.
     */
    private function encodeIt()
    {
        $this->prepare();
        $str = "";
        for ($i = 0; $i < strlen($this->code); ++$i)
            $str .= $this->hashIt(base_convert(ord($this->code[$i]) + $this->interval, 10, $this->option)) . $this->mask[$this->option];
        return $str;
    }


    /**
     * @param $Code
     * @param $html
     * @param string $Code The JavaScript code to be obfuscated.
     * @param bool $html Indicates whether the provided code is HTML.
     * @return string Obfuscates the provided JavaScript code.
     *
     * @return string The obfuscated JavaScript code.
     */
    public function obfuscate($Code,$html = false)
    {
        if ($html) {
            $Code = $this->cleanHtml($Code);
            $this->code = $this->html2Js($Code);
        } else {
            $Code = $this->cleanJS($Code);
            $this->code = $Code;
        }
        $rand = rand(0,99);
        $rand1 = rand(0,99);
        return "var _0xc{$rand}e=[\"\",\"\x73\x70\x6C\x69\x74\",\"\",\"\x73\x6C\x69\x63\x65\",\"\x69\x6E\x64\x65\x78\x4F\x66\",\"\",\"\",\"\x2E\",\"\x70\x6F\x77\",\"\x72\x65\x64\x75\x63\x65\",\"\x72\x65\x76\x65\x72\x73\x65\",\"\x30\"];function _0xe{$rand1}c(d,e,f){var g=_0xc{$rand}e[2][_0xc{$rand}e[1]](_0xc{$rand}e[0]);var h=g[_0xc{$rand}e[3]](0,e);var i=g[_0xc{$rand}e[3]](0,f);var j=d[_0xc{$rand}e[1]](_0xc{$rand}e[0])[_0xc{$rand}e[10]]()[_0xc{$rand}e[9]](function(a,b,c){if(h[_0xc{$rand}e[4]](b)!==-1)return a+=h[_0xc{$rand}e[4]](b)*(Math[_0xc{$rand}e[8]](e,c))},0);var k=_0xc{$rand}e[0];while(j>0){k=i[j%f]+k;j=(j-(j%f))/f}return k||_0xc{$rand}e[11]}eval(function(h,u,n,t,e,r){r=\"\";for(var i=0,len=h.length;i<len;i++){var s=\"\";while(h[i]!==n[e]){s+=h[i];i++}for(var j=0;j<n.length;j++)s=s.replace(new RegExp(n[j],\"g\"),j);r+=String.fromCharCode(_0xe{$rand1}c(s,e,10)-t)}return decodeURIComponent(escape(r))}(\"" . $this->encodeIt() . "\"," . rand(1, 100) . ",\"" . $this->mask . "\"," . $this->interval . "," . $this->option . "," . rand(1, 60) . "))";
    }

    /**
     * Sets the expiration time for the obfuscated code.
     *
     * @param string $expireTime The expiration time in a format compatible with strtotime.
     * @return bool True if the expiration time is set successfully, false otherwise.
     */
    public function setExpiration($expireTime)
    {
        if (strtotime($expireTime)) {
            $this->expireTime = strtotime($expireTime);
            return true;
        }
        return false;
    }

    /**
     * Adds a domain name to the list for restricting code execution.
     *
     * @param string $domainName The domain name to be added.
     * @return bool True if the domain name is valid and added successfully, false otherwise.
     */
    public function addDomainName($domainName)
    {
        if ($this->isValidDomain($domainName)) {
            $this->domainNames[] = $domainName;
            return true;
        }
        return false;
    }

    /**
     * Checks if the provided domain name is valid.
     *
     * @param string $domain_name The domain name to be validated.
     * @return bool True if the domain name is valid, false otherwise.
     */
    private function isValidDomain($domain_name)
    {
        return (preg_match("/^([a-z\d](-*[a-z\d])*)(\.([a-z\d](-*[a-z\d])*))*$/i", $domain_name)
            && preg_match("/^.{1,253}$/", $domain_name)
            && preg_match("/^[^\.]{1,63}(\.[^\.]{1,63})*$/", $domain_name));
    }

    /**
     * Converts HTML code to JavaScript code for document.write.
     *
     * @param string $code The HTML code to be converted.
     * @return string The resulting JavaScript code.
     */
    private function html2Js($code)
    {
        $search = array(
            '/\>[^\S ]+/s',     // strip whitespaces after tags, except space
            '/[^\S ]+\</s',     // strip whitespaces before tags, except space
            '/(\s)+/s',         // shorten multiple whitespace sequences
            '/<!--(.|\s)*?-->/' // Remove HTML comments
        );
        $replace = array(
            '>',
            '<',
            '\\1',
            ''
        );
        $code = preg_replace($search, $replace, $code);
        $code = "document.write('" . addslashes($code . " ") . "');";
        return $code;
    }

    /**
     * Removes HTML comments from the provided code.
     *
     * @param string $code The code containing HTML comments.
     * @return string The code with HTML comments removed.
     */
    private function cleanHtml($code)
    {
        return preg_replace('/<!--(.|\s)*?-->/', '', $code);
    }

    /**
     * Removes JavaScript comments and unnecessary whitespaces from the provided code.
     *
     * @param string $code The JavaScript code to be cleaned.
     * @return string The cleaned JavaScript code.
     */
    private function cleanJS($code)
    {
        $pattern = '/(?:(?:\/\*(?:[^*]|(?:\*+[^*\/]))*\*+\/)|(?:(?<!\:|\\\|\')\/\/.*))/';
        $code = preg_replace($pattern, '', $code);
        $search = array(
            '/\>[^\S ]+/s',     // strip whitespaces after tags, except space
            '/[^\S ]+\</s',     // strip whitespaces before tags, except space
            '/(\s)+/s',         // shorten multiple whitespace sequences
            '/<!--(.|\s)*?-->/' // Remove HTML comments
        );
        $replace = array(
            '>',
            '<',
            '\\1',
            ''
        );
        return preg_replace($search, $replace, $code);
    }
}